...
exports.validateObjectIdArray = exports.validateObjectIdInBody = exports.validateMultipleObjectIds = exports.validateObjectId = exports.validateBugReportUpdate = exports.optionalAuthenticate = exports.validateBugReport = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
function parseFormDataField(field) {
    if (!field)
        return undefined;
...
const validateBugReport = (req, res, next) => {
    try {
        ...
        const { feedbackType = 'bug_report', title, description, category, urgencyLevel, bugType, } = req.body;
        ...
        if (!title || typeof title !== 'string' || title.trim().length < 2) {
            const error = new Error('Title is required and must be at least 2 characters');
            error.statusCode = 400;
            throw error;
        }
        if (!description || typeof description !== 'string' || description.trim().length < 2) {
            const error = new Error('Description is required and must be at least 2 characters');
            error.statusCode = 400;
            throw error;
        }
        ...
        if (!validFeedbackTypes.includes(feedbackType)) {
            const error = new Error(`Invalid feedback type. Must be one of: ${validFeedbackTypes.join(', ')}`);
            error.statusCode = 400;
            throw error;
        }
        // Validate category
        ...
        if (urgencyLevel) {
            ...
            if (!validUrgencyLevels.includes(urgencyLevel)) {
                const error = new Error(`Invalid urgency level. Must be one of: ${validUrgencyLevels.join(', ')}`);
                error.statusCode = 400;
                throw error;
            }
        }
        ...
        if (bugType) {
            ...
            if (!validEffortLevels.includes(bugType)) {
                const error = new Error(`Invalid estimated effort. Must be one of: ${validEffortLevels.join(', ')}`);
                error.statusCode = 400;
                throw error;
            }
        }
 ...
 */
function validateBugReportSpecific(body) {
    ...
    if (!steps || typeof steps !== 'string' || steps.trim().length < 5) {
        const error = new Error('Steps to reproduce are required for bug reports and must be descriptive');
        error.statusCode = 400;
        throw error;
    }
    if (!expectedBehavior || typeof expectedBehavior !== 'string' || expectedBehavior.trim().length < 5) {
        const error = new Error('Expected behavior is required for bug reports and must be descriptive');
        error.statusCode = 400;
        throw error;
    }
    if (!actualBehavior || typeof actualBehavior !== 'string' || actualBehavior.trim().length < 5) {
        const error = new Error('Actual behavior is required for bug reports and must be descriptive');
        error.statusCode = 400;
        throw error;
    }
}
 ...
 */
function validateUserExperienceSpecific(body) {
 ...
 */
function validateThematicFeedbackSpecific(body) {
    const { thematicFeedback } = body;
    if (!thematicFeedback ||
        (!thematicFeedback.lookAndFeelRating && !thematicFeedback.specificThematicComments)) {
        const error = new Error('Thematic feedback requires either a look & feel rating or specific comments');
        error.statusCode = 400;
        throw error;
    }
}
 ...
 */
function validateFeatureSuggestionSpecific(body) {
    ...
    if (!featureSuggestion ||
        ...
        const error = new Error('Feature description is required and must be at least 10 characters');
        error.statusCode = 400;
        throw error;
    }
}
 ...
 */
function validateUserExperienceRating(rating) {
    if (!rating || typeof rating !== 'object') {
        const error = new Error('User experience rating must be an object');
        error.statusCode = 400;
        throw error;
    }
    ...
    for (const field of ratingFields) {
        if (rating[field] !== undefined && rating[field] !== null && rating[field] !== '') {
            ...
            if (isNaN(numericValue) || numericValue < 1 || numericValue > 5) {
                const error = new Error(`${field} must be a number between 1 and 5`);
                error.statusCode = 400;
                throw error;
            }
    ...
    if (!overallSat || overallSat === 0 || isNaN(overallSat)) {
        const error = new Error('Overall satisfaction rating is required');
        error.statusCode = 400;
        throw error;
    }
 ...
 */
function validateThematicFeedbackData(feedback) {
    ...
    if (feedback.colorSchemeAppropriate !== undefined && typeof feedback.colorSchemeAppropriate !== 'boolean') {
        const error = new Error('Color scheme appropriate must be a boolean value');
        error.statusCode = 400;
        throw error;
    }
    if (feedback.specificThematicComments && typeof feedback.specificThematicComments !== 'string') {
        const error = new Error('Specific thematic comments must be a string');
        error.statusCode = 400;
        throw error;
    }
}
 ...
 */
function validatePerformanceIssues(performance) {
    if (performance.pageLoadTime !== undefined) {
        if (typeof performance.pageLoadTime !== 'number' || performance.pageLoadTime < 0) {
            const error = new Error('Page load time must be a positive number');
            error.statusCode = 400;
            throw error;
        }
    }
    if (performance.timeToInteractive !== undefined) {
        if (typeof performance.timeToInteractive !== 'number' || performance.timeToInteractive < 0) {
            const error = new Error('Time to interactive must be a positive number');
            error.statusCode = 400;
            throw error;
        }
    }
    if (performance.specificSlowAreas && !Array.isArray(performance.specificSlowAreas)) {
        const error = new Error('Specific slow areas must be an array');
        error.statusCode = 400;
        throw error;
    }
    if (performance.browserFreeze !== undefined && typeof performance.browserFreeze !== 'boolean') {
        const error = new Error('Browser freeze must be a boolean value');
        error.statusCode = 400;
        throw error;
    }
    if (performance.memoryIssues !== undefined && typeof performance.memoryIssues !== 'boolean') {
        const error = new Error('Memory issues must be a boolean value');
        error.statusCode = 400;
        throw error;
    }
}
...
function validateBusinessImpact(impact) {
    ...
    for (const field of booleanFields) {
        if (impact[field] !== undefined && typeof impact[field] !== 'boolean') {
            const error = new Error(`${field} must be a boolean value`);
            error.statusCode = 400;
            throw error;
        }
    }
...
function validateSystemInfo(systemInfo) {
    ...
    try {
        ...
        const customError = new Error('System info must be valid JSON');
    ...
    for (const field of requiredFields) {
        if (!parsedSystemInfo[field] || typeof parsedSystemInfo[field] !== 'string') {
            const error = new Error(`System info must include valid ${field}`);
            error.statusCode = 400;
            throw error;
        }
    }
    ...
    try {
        ...
        const customError = new Error('System info URL must be a valid URL');
        customError.statusCode = 400;
        throw customError;
    }
    ...
    if (parsedSystemInfo.deviceType) {
        ...
        if (!validDeviceTypes.includes(parsedSystemInfo.deviceType)) {
            const error = new Error(`Device type must be one of: ${validDeviceTypes.join(', ')}`);
            error.statusCode = 400;
            throw error;
        }
    }
...
function validateTags(tags) {
    ...
    for (const tag of tags) {
        if (typeof tag !== 'string' || tag.trim().length === 0) {
            const error = new Error('Each tag must be a non-empty string');
            error.statusCode = 400;
            throw error;
        }
        if (tag.length > 50) {
            const error = new Error('Each tag must be 50 characters or less');
            error.statusCode = 400;
            throw error;
        }
    }
...
function validateFileUploads(files) {
    if (files.length > 5) {
        const error = new Error('Maximum 5 files allowed per report');
        error.statusCode = 400;
        throw error;
    }
    for (const file of files) {
        ...
        if (file.size > 10 * 1024 * 1024) { // 10MB
            const error = new Error(`File ${file.originalname} is too large. Maximum size is 10MB`);
            error.statusCode = 400;
            throw error;
        }
        // Check filename length
        if (file.originalname.length > 100) {
            const error = new Error(`Filename ${file.originalname} is too long. Maximum 100 characters`);
            error.statusCode = 400;
            throw error;
        }
    }
 ...
 */
const validateBugReportUpdate = (req, res, next) => {
    try {
        const { status, priority, urgencyLevel, estimatedEffort, businessImpact, tags, resolution } = req.body;
        ...
        if (resolution && (typeof resolution !== 'string' || resolution.trim().length < 5)) {
            const error = new Error('Resolution must be at least 5 characters when provided');
            error.statusCode = 400;
            throw error;
        }
        next();
...
exports.validateBugReportUpdate = validateBugReportUpdate;
/**
...
const validateObjectId = (paramName) => {
    return (req, res, next) => {
        ...
        if (!id) {
            const error = new Error(`${paramName} parameter is required`);
            error.statusCode = 400;
            return next(error);
        }
 ...
 * @returns Express middleware function
 */
const validateObjectIdInBody = (fieldName, required = true) => {
    return (req, res, next) => {
...
exports.validateObjectIdInBody = validateObjectIdInBody;
/**
 ...
 * @returns Express middleware function
 */
const validateObjectIdArray = (fieldName, required = true) => {
    return (req, res, next) => {
        ...
        for (let i = 0; i < ids.length; i++) {
            if (!mongoose_1.default.Types.ObjectId.isValid(ids[i])) {
                const error = new Error(`Invalid ObjectId at index ${i} in ${fieldName}`);
                error.statusCode = 400;
                return next(error);
            }
        }
