...
exports.CollaborationEvent = exports.UserSession = exports.UserActivity = exports.UserActivityTrackingService = void 0;
...
const collaborationEventSchema = new mongoose_1.default.Schema({
    reportId: {
        type: mongoose_1.default.Schema.Types.ObjectId,
        ref: 'Report',
        required: true,
...
const CollaborationEvent = mongoose_1.default.model('CollaborationEvent', collaborationEventSchema);
exports.CollaborationEvent = CollaborationEvent;
class UserActivityTrackingService {
    /**
     * Log user activity
     */
    static logActivity(reportId_1, userId_1, activityType_1, action_1) {
        return __awaiter(this, arguments, void 0, function* (reportId, userId, activityType, action, options = {}) {
            try {
                const activity = new UserActivity({
                    reportId: new mongoose_1.default.Types.ObjectId(reportId),
                ...
                if (options.sessionId) {
                    yield this.updateSession(options.sessionId, userId, reportId);
                }
     ...
     */
    static startSession(reportId, userId, sessionId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                // Check if session already exists
                let session = yield UserSession.findOne({ sessionId });
                if (session) {
    ...
    static endSession(sessionId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const session = yield UserSession.findOne({ sessionId, isActive: true });
                if (session) {
     ...
     */
    static getReportActivity(reportId_1) {
        return __awaiter(this, arguments, void 0, function* (reportId, options = {}) {
            try {
                ...
                // Execute queries
                const [activities, totalCount, summary] = yield Promise.all([
     ...
     */
    static getUserActivitySummary(userId_1) {
        return __awaiter(this, arguments, void 0, function* (userId, options = {}) {
            try {
                ...
                const [activitiesAgg, recentActivities] = yield Promise.all([
                ...
                const reportDetails = yield report_model_1.default.find({
                    _id: { $in: topReportIds }
                }).select('_id title');
                const reportDetailsMap = new Map();
                reportDetails.forEach(report => {
                    reportDetailsMap.set(report._id.toString(), report.title);
                });
     ...
     */
    static trackCollaboration(reportId, eventType, participants) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const collaborationEvent = new CollaborationEvent({
                    ...
                    participants: participants.map(p => ({
                        userId: new mongoose_1.default.Types.ObjectId(p.userId),
                        role: p.role,
                        joinedAt: new Date(),
                        contributionScore: 0
                    })),
                ...
                yield collaborationEvent.save();
                return collaborationEvent.toObject();
            }
            catch (error) {
     ...
     */
    static endCollaboration(collaborationEventId, summary) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const event = yield CollaborationEvent.findById(collaborationEventId);
                if (event) {
     ...
     */
    static getCollaborationAnalytics(reportId_1) {
        return __awaiter(this, arguments, void 0, function* (reportId, options = {}) {
            try {
                const query = { reportId };
                if (options.fromDate || options.toDate) {
                ...
                const events = yield CollaborationEvent.find(query)
     ...
     */
    static cleanupOldActivity() {
        return __awaiter(this, arguments, void 0, function* (retentionDays = 365) {
            try {
                const cutoffDate = new Date(Date.now() - (retentionDays * 24 * 60 * 60 * 1000));
                const [activitiesResult, sessionsResult, collaborationResult] = yield Promise.all([
    ...
    static updateSession(sessionId, userId, reportId) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const session = yield UserSession.findOne({ sessionId });
                if (session) {
                    session.pageViews += 1;
                    yield session.save();
                }
    ...
    static generateActivitySummary(reportId, options) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const query = { reportId };
                if (options.fromDate || options.toDate) {
                ...
                const summary = yield UserActivity.aggregate([
                    { $match: query },
                    {
                        $group: {
                            _id: {
...
exports.UserActivityTrackingService = UserActivityTrackingService;
exports.default = UserActivityTrackingService;
//# sourceMappingURL=userActivityTracking.service.js.map