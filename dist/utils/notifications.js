...
exports.sendBugReportNotification = void 0;
 ...
 */
const sendBugReportNotification = (bugReport) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        // Get all notification emails from env
        const allNotificationEmails = env_1.env.BUG_REPORT_NOTIFICATION_EMAILS
            .split(',')
        ...
        const result = yield email_service_1.default.sendEmail(Object.assign({ to: recipients, subject,
            html }, (bugReport.attachments && bugReport.attachments.length > 0 && {
...
exports.sendBugReportNotification = sendBugReportNotification;
/**
 * Generate subject line based on bug report type and priority
 */
function generateSubjectLine(bugReport) {
    const typeMap = {
 ...
 */
function generateBugReportHTML(bugReport) {
 ...
 */
function generateBugReportSection(bugReport) {
    return `
    <!-- Bug Report Details -->
...
function generateUserExperienceSection(bugReport) {
    if (!bugReport.userExperienceRating)
        return '';
...
function generateThematicFeedbackSection(bugReport) {
    if (!bugReport.thematicFeedback)
        return '';
...
function generateFeatureSuggestionSection(bugReport) {
    if (!bugReport.featureSuggestion)
        return '';
...
function generateGeneralFeedbackSection(bugReport) {
    return `
    <!-- General Feedback -->
...
function generateBusinessImpactSection(bugReport) {
    if (!bugReport.businessImpact)
        return '';
...
function generateSystemInfoSection(bugReport) {
...
function generateReporterInfoSection(bugReport) {
    const system = bugReport.systemInfo;
    return `
...
function generateAttachmentsSection(bugReport) {
...
function generateActionSection(bugReport) {
    return `
    <!-- Action Required -->
    <div style="background-color: #1f2937; color: white; padding: 20px; border-radius: 8px; margin: 20px 0; text-align: center;">
      ...
      <a href="${env_1.env.FRONTEND_URL}/admin/bug-reports/${bugReport._id}" 
 ...
 */
function generateStatusSection(bugReport) {
    const statusColors = {
        'new': '#10b981', // Green
        'triaged': '#3b82f6', // Blue
 ...
 */
function generateVerificationSection(bugReport) {
    if (!bugReport.verified || !bugReport.verificationDetails)
        return '';
    return `
...
function generateAssignmentAndSourceSection(bugReport) {
    return `
 ...
 */
function getUrgencyDisplay(urgencyLevel) {
    const urgencyLabels = {
    ...
    return urgencyLabels[urgencyLevel] || urgencyLevel;
}
...
function getFeedbackTypeTitle(type) {
    const titles = {
        'bug_report': 'Bug Report',
...
function generatePerformanceIssuesSection(performance) {
    return `
    <div style="margin-top: 15px; padding: 15px; background-color: white; border-radius: 4px;">
