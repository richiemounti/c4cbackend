...
exports.mapCSVTasksToSchema = exports.loadProjectSiteSetupTasks = exports.loadProjectSetupTasks = exports.convertCSVDataToSetupTasks = exports.parseCSVFile = void 0;
// utils/csvParser.ts
...
const parseCSVFile = (filePath) => {
    const fileContent = fs_1.default.readFileSync(path_1.default.resolve(filePath), { encoding: 'utf-8' });
    const records = (0, sync_1.parse)(fileContent, { columns: true, skip_empty_lines: true, trim: true });
...
const mapDataType = (csvDataType) => {
    ...
    if (type === 'array' || type === 'list' || type === 'enum' ||
 ...
 */
const isNonFieldRow = (fieldName) => {
    if (!fieldName)
        return true;
    if (fieldName === 'Field Name')
        return true; // header row
    if (/^[A-D]\.\s+/.test(fieldName))
        return true; // "A. Access", "B. Safeguarding" subsections
    if (/^If\s+(Yes|No):/i.test(fieldName))
        return true; // "If Yes:"
    if (/^\d+\.\s+/.test(fieldName))
        return true; // "1. Core Principle" explanatory rows
    return false;
};
/**
 ...
 */
const parseStepHeader = (fieldName, currentStepNumber) => {
    // "Step N — Label"
 ...
 */
const convertCSVDataToSetupTasks = (csvData, isProjectSite = false) => {
    const tasks = [];
    ...
    for (const row of dataRows) {
        ...
        if (conditionalOn)
            task.conditionalOn = conditionalOn;
        tasks.push(task);
    }
    return tasks;
};
exports.convertCSVDataToSetupTasks = convertCSVDataToSetupTasks;
// ---------------------------------------------------------------------------
// Legacy helpers (kept for any existing callers outside the seed script)
// ---------------------------------------------------------------------------
const loadProjectSetupTasks = () => {
    const filePath = path_1.default.join(__dirname, '../data/Set_Up_Your_Project_final.csv');
    return (0, exports.parseCSVFile)(filePath);
};
exports.loadProjectSetupTasks = loadProjectSetupTasks;
const loadProjectSiteSetupTasks = () => {
    const filePath = path_1.default.join(__dirname, '../data/Set_Up_Your_Sites_final.csv');
    return (0, exports.parseCSVFile)(filePath);
};
exports.loadProjectSiteSetupTasks = loadProjectSiteSetupTasks;
const mapCSVTasksToSchema = (tasks, step) => tasks.map((task, index) => {
    var _a;
    const mapped = {
    ...
    if ((_a = task.options) === null || _a === void 0 ? void 0 : _a.length)
        mapped.options = task.options;
    return mapped;
});
exports.mapCSVTasksToSchema = mapCSVTasksToSchema;
