...
exports.getChangeStats = exports.getMitigationEffectiveness = exports.getStatusChanges = exports.getRiskTrends = exports.getRiskChangelog = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
const riskChangeLog_model_1 = __importDefault(require("../models/riskChangeLog.model"));
 ...
 */
const getRiskChangelog = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const changeLogs = yield riskChangeLog_model_1.default.find({ riskId })
...
exports.getRiskChangelog = getRiskChangelog;
/**
 * Get risk trends over time for visualizations
 * @route GET /api/v1/admin/dashboard/risks/analytics/trends
 * @access Private (Manager, ProjectCreator, Organiser, Reviewer)
 */
const getRiskTrends = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!isUserAuthenticated(req)) {
            const error = new Error('Authentication required');
            error.statusCode = 401;
        ...
        const changeLogs = yield riskChangeLog_model_1.default.find({
            project: new mongoose_1.default.Types.ObjectId(projectId),
            ...
            risks.forEach(risk => {
                if (new Date(risk.createdAt) <= currentDate) {
                    ...
                    changeLogs.forEach(log => {
                        if (log.riskId.toString() === risk._id.toString() &&
                            ...
                            log.changes.forEach(change => {
                                if (change.field === 'riskScore')
                                    currentScore = change.newValue;
                    ...
                    if (currentScore === 'high')
                        snapshot.highRisks++;
                    else if (currentScore === 'medium')
                        snapshot.mediumRisks++;
                    else if (currentScore === 'low')
                        snapshot.lowRisks++;
                    // Count by status
 ...
 * @access Private (Manager, ProjectCreator, Organiser, Reviewer)
 */
const getStatusChanges = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!isUserAuthenticated(req)) {
            const error = new Error('Authentication required');
            error.statusCode = 401;
        ...
        const start = startDate
            ...
            : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
        const end = endDate ? new Date(endDate) : new Date();
        const statusChanges = yield riskChangeLog_model_1.default.aggregate([
            {
                $match: {
                    project: new mongoose_1.default.Types.ObjectId(projectId),
                    changeType: 'status',
...
exports.getStatusChanges = getStatusChanges;
/**
 * Get mitigation effectiveness analysis
 * @route GET /api/v1/admin/dashboard/risks/analytics/mitigation-effectiveness
 * @access Private (Manager, ProjectCreator, Organiser, Reviewer)
 */
const getMitigationEffectiveness = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!isUserAuthenticated(req)) {
            const error = new Error('Authentication required');
            error.statusCode = 401;
        ...
        const effectiveness = risks.map(risk => {
            ...
            const daysTracked = Math.floor((Date.now() - new Date(risk.createdAt).getTime()) / (1000 * 60 * 60 * 24));
            // Count completed actions
            const actionsCompleted = risk.mitigationActions.filter(a => a.status === 'completed').length;
            const totalActions = risk.mitigationActions.length;
            // Calculate effectiveness score (0-100)
            const effectiveness = calculateEffectiveness(initialScore, risk.riskScore, actionsCompleted, totalActions);
            return {
                riskId: risk._id,
                riskName: risk.name,
                initialScore,
...
exports.getMitigationEffectiveness = getMitigationEffectiveness;
/**
 * Calculate effectiveness score based on risk improvement and action completion
 */
function calculateEffectiveness(initialScore, currentScore, completed, total) {
    const scoreValues = {
        low: 1,
        medium: 2,
        high: 3
 ...
 * @access Private (Manager, ProjectCreator, Organiser, Reviewer)
 */
const getChangeStats = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        if (!isUserAuthenticated(req)) {
            const error = new Error('Authentication required');
            error.statusCode = 401;
        ...
        const start = startDate
            ...
            : new Date(Date.now() - 90 * 24 * 60 * 60 * 1000);
        const end = endDate ? new Date(endDate) : new Date();
        const stats = yield riskChangeLog_model_1.default.getChangeStats(new mongoose_1.default.Types.ObjectId(projectId), start, end);
        // Get most active users
        const activeUsers = yield riskChangeLog_model_1.default.aggregate([
            {
                $match: {
                    project: new mongoose_1.default.Types.ObjectId(projectId),
                    changedAt: { $gte: start, $lte: end }
...
exports.default = {
    getRiskChangelog: exports.getRiskChangelog,
    getRiskTrends: exports.getRiskTrends,
    getStatusChanges: exports.getStatusChanges,
    getMitigationEffectiveness: exports.getMitigationEffectiveness,
    getChangeStats: exports.getChangeStats
};
//# sourceMappingURL=riskAnalytics.controller.js.map