...
exports.updateImpact = exports.getAvailableSubThemes = exports.getImpactsByStage = exports.getImpactRisks = exports.getImpactById = exports.getImpactsByStakeholder = exports.deleteImpact = exports.defineOutcome = void 0;
 ...
 * @access Private
 */
const defineOutcome = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (stage.stageNumber !== 2) {
            const error = new Error('Social impacts can only be added to Stage 2');
            error.statusCode = 400;
            throw error;
        }
        ...
        const invalidStageThemes = themes.filter(t => t.theoryOfChangeStage !== 'Stage 2 - Outcome' && t.theoryOfChangeStage !== 'Both');
        if (invalidStageThemes.length > 0) {
            const error = new Error(`Themes [${invalidStageThemes.map(t => t.name).join(', ')}] are not scoped to Stage 2. ` +
        ...
        const invalidStageSubThemes = subThemes.filter(st => st.theoryOfChangeStage !== 'Stage 2 - Outcome');
        if (invalidStageSubThemes.length > 0) {
            const error = new Error(`SubThemes [${invalidStageSubThemes.map(st => st.name).join(', ')}] are not scoped to Stage 2.`);
            error.statusCode = 400;
            throw error;
        }
        ...
        const existingImpact = yield socialImpact_model_1.default.findOne({
            project: projectId,
        ...
        if (existingImpact) {
            const error = new Error('A social impact with the same outcome already exists for this stakeholder group');
            error.statusCode = 409;
            throw error;
        }
        // CHANGED: Create the social impact with multiple themes and subthemes
        // ✅ FIXED: Use create with array syntax
        const newImpact = yield socialImpact_model_1.default.create([{
                project: projectId,
                projectSite: projectSiteId || null,
                stage: stageId,
                stakeholderGroup: stakeholderGroupId,
        ...
        const populatedImpact = yield socialImpact_model_1.default.findById(newImpact[0]._id)
...
exports.defineOutcome = defineOutcome;
/**
 * Delete a social impact
 * @route DELETE /api/v1/socialImpacts/:impactId
 * @access Private
 */
const deleteImpact = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        if (!isUserAuthenticated(req)) {
        ...
        const impact = yield socialImpact_model_1.default.findById(impactId).session(session);
        if (!impact) {
            const error = new Error('Social impact not found');
            error.statusCode = 404;
            throw error;
        }
 ...
 * @access Private
 */
const getImpactsByStakeholder = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
...
exports.getImpactsByStakeholder = getImpactsByStakeholder;
/**
 * Get a single social impact by ID
 * @route GET /api/v1/socialImpacts/:impactId
 * @access Private
 */
const getImpactById = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { impactId } = req.params;
        const impact = yield socialImpact_model_1.default.findById(impactId)
            .populate('stakeholderGroup', 'name')
            .populate('themes', 'name')
            .populate('subThemes', 'name')
            .populate('creator', 'name')
        ...
        // Handle invalid MongoDB ID format
        if (error instanceof Error && error.name === 'CastError') {
            const customError = new Error('Invalid impact ID format');
            customError.statusCode = 400;
            return next(customError);
        }
        next(error);
...
exports.getImpactById = getImpactById;
/**
 * Get all RiskRegister entries linked to a social impact outcome
 * @route GET /api/v1/toc/impacts/:impactId/risks
 * @access Private
 */
const getImpactRisks = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { impactId } = req.params;
        const impact = yield socialImpact_model_1.default.findById(impactId);
        if (!impact || impact.archived) {
        ...
        const risks = yield riskRegister_model_1.default.find({
...
exports.getImpactRisks = getImpactRisks;
/**
 * Get all social impacts for a specific Theory of Change stage
 * @route GET /api/v1/socialImpacts/stage/:stageId
 * @access Private
 */
const getImpactsByStage = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        // Verify it's stage 2
        if (stage.stageNumber !== 2) {
            const error = new Error('Social impacts are only available in Stage 2');
            error.statusCode = 400;
            throw error;
        }
        ...
        const allResilienceTags = new Set();
        impacts.forEach(impact => {
            impact.themes.forEach((theme) => allThemes.add(theme.name));
            impact.subThemes.forEach((subTheme) => allSubThemes.add(subTheme.name));
            impact.sdgTags.forEach(tag => allSdgTags.add(tag));
            impact.resilienceTags.forEach(tag => allResilienceTags.add(tag));
        });
        res.status(200).json({
...
exports.getImpactsByStage = getImpactsByStage;
/**
...
const getAvailableSubThemes = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const aggregatedSdgTags = new Set();
        const aggregatedResilienceTags = new Set();
        availableSubThemes.forEach(subTheme => {
            if (subTheme.sdgTags) {
                subTheme.sdgTags.forEach((tag) => aggregatedSdgTags.add(tag));
            }
            if (subTheme.resilienceTags) {
                subTheme.resilienceTags.forEach((tag) => aggregatedResilienceTags.add(tag));
            }
        });
        res.status(200).json({
            success: true,
 ...
 * @access Private
 */
const updateImpact = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    const session = yield mongoose_1.default.startSession();
    session.startTransaction();
    try {
        if (!isUserAuthenticated(req)) {
        ...
        const { impactId } = req.params;
        const { themeIds, subThemeIds, outcome, notes } = req.body;
        const existingImpact = yield socialImpact_model_1.default.findById(impactId);
        if (!existingImpact) {
            const error = new Error('Social impact not found');
            error.statusCode = 404;
            throw error;
        ...
        if (themeIds && subThemeIds) {
            ...
            const isValidRelationship = yield (0, theoryOfChange_service_1.validateMultipleStakeholderThemeRelationships)(existingImpact.stakeholderGroup.toString(), themeIds);
            if (!isValidRelationship) {
                const error = new Error('Invalid stakeholder-theme relationship for one or more selected themes');
                error.statusCode = 400;
                throw error;
            ...
            const invalidStageThemes = updatedThemes.filter(t => t.theoryOfChangeStage !== 'Stage 2 - Outcome' && t.theoryOfChangeStage !== 'Both');
            if (invalidStageThemes.length > 0) {
                const error = new Error(`Themes [${invalidStageThemes.map(t => t.name).join(', ')}] are not scoped to Stage 2.`);
                error.statusCode = 400;
                throw error;
        ...
        if (outcome !== undefined)
            existingImpact.outcome = outcome;
        if (notes !== undefined)
            existingImpact.notes = notes;
        existingImpact.lastUpdatedBy = req.user._id;
        yield existingImpact.save({ session });
        yield session.commitTransaction();
        session.endSession();
        const populatedImpact = yield socialImpact_model_1.default.findById(impactId)
            .populate('stakeholderGroup', 'name')
            .populate('themes', 'name')
            .populate('subThemes', 'name')
            .populate('creator', 'name')
...
//# sourceMappingURL=socialImpact.controller.js.map