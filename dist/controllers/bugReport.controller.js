...
exports.getBugReportAnalytics = exports.verifyBugReport = exports.updateBugReport = exports.getBugReport = exports.getBugReports = exports.submitBugReport = void 0;
 ...
 * @access Public
 */
const submitBugReport = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const { 
        // Basic fields
        feedbackType = 'bug_report', title, description, category, subCategory, 
        // Bug-specific fields
        steps, expectedBehavior, actualBehavior, 
        // Assessment fields
        urgencyLevel = 'fix_this_week', bugType, 
        // NEW FIELDS:
        assignedToTeamMember, sourceOfFeedback, 
        // Device context
        deviceContext, 
        // Additional fields
        tags = [], requiresFollowUp = false, followUpDate } = req.body;
        ...
        if (!category) {
            const error = new Error('Category is required for proper classification');
            error.statusCode = 400;
            throw error;
        }
        ...
        if (parsedSourceOfFeedback && (!parsedSourceOfFeedback.source || parsedSourceOfFeedback.source.trim() === '')) {
            const error = new Error('Source of feedback must include a source description');
            error.statusCode = 400;
            throw error;
        }
        // Validate assignedToTeamMember if provided
        if (assignedToTeamMember && !['kate', 'sam', 'belinda'].includes(assignedToTeamMember)) {
            const error = new Error('Invalid team member assignment');
            error.statusCode = 400;
            throw error;
        }
        ...
        switch (feedbackType) {
            case 'bug_report':
                if (!steps || !expectedBehavior || !actualBehavior) {
                    const error = new Error('Steps, expected behavior, and actual behavior are required for bug reports');
                    error.statusCode = 400;
                    throw error;
                }
                break;
            case 'user_experience':
                if (!userExperienceRating || !userExperienceRating.overallSatisfaction || userExperienceRating.overallSatisfaction === 0) {
                    const error = new Error('Overall satisfaction rating is required for user experience feedback');
                    error.statusCode = 400;
                    throw error;
                }
                break;
            case 'feature_suggestion':
                if (!featureSuggestion || !featureSuggestion.description || !featureSuggestion.description.trim()) {
                    const error = new Error('Feature description is required for feature suggestions');
                    error.statusCode = 400;
                    throw error;
                }
                break;
        ...
        if (parsedSystemInfo && (!parsedSystemInfo.url || !parsedSystemInfo.userAgent)) {
            const customError = new Error('Invalid system information provided');
        ...
        if (req.file) {
            try {
                ...
                const uploadResult = yield (0, cloudinaryStorage_service_1.uploadFile)(req.file, 'bug-reports');
        ...
        if (req.files && Array.isArray(req.files)) {
            for (const file of req.files) {
                try {
                    ...
                    const uploadResult = yield (0, cloudinaryStorage_service_1.uploadFile)(file, 'bug-reports');
        ...
        if (urgencyLevel === 'fix_24_hours' || (businessImpact === null || businessImpact === void 0 ? void 0 : businessImpact.functionalityBlocked)) {
            autoPriority = 'p0';
        }
        else if (urgencyLevel === 'fix_1_3_days' || (businessImpact === null || businessImpact === void 0 ? void 0 : businessImpact.revenueImpact) || (businessImpact === null || businessImpact === void 0 ? void 0 : businessImpact.complianceImpact)) {
            autoPriority = 'p1';
        }
        else if (urgencyLevel === 'fix_this_week' || (businessImpact === null || businessImpact === void 0 ? void 0 : businessImpact.affectedUsers) === 'most' || (businessImpact === null || businessImpact === void 0 ? void 0 : businessImpact.affectedUsers) === 'all') {
            autoPriority = 'p2';
        }
        ...
        const bugReportData = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ 
            // Basic information
            feedbackType,
            title,
            description,
            ...
            subCategory, 
            // Assessment fields
            urgencyLevel: urgencyLevel }, (cleanEstimatedEffort && { bugType: cleanEstimatedEffort })), { priority: autoPriority }), (assignedToTeamMember && { assignedToTeamMember })), (parsedSourceOfFeedback && { sourceOfFeedback: parsedSourceOfFeedback })), { 
            // Context and tracking
            deviceContext, systemInfo: parsedSystemInfo, 
            // Files and attachments
            screenshot: screenshotUrl, attachments: processedAttachments, 
            // Additional metadata
            tags: Array.isArray(parsedTags) ? parsedTags : [], requiresFollowUp }), (followUpDate && { followUpDate: new Date(followUpDate) })), (req.user && { reporter: req.user._id })), { 
            // Initialize metrics
            metrics: {
                viewCount: 0,
                commentCount: 0,
            ...
            // Default status
            status: 'new', resolved: false, verified: false, verifiedByReporter: false });
        ...
        const bugReport = new bugReport_model_1.default(bugReportData);
        const savedBugReport = yield bugReport.save();
        // Send notification to admin/developer team
        try {
            yield (0, notifications_1.sendBugReportNotification)(savedBugReport);
        }
 ...
 */
const getBugReports = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        ...
        const affectedUsers = req.query.affectedUsers;
        const assignedToTeamMember = req.query.assignedToTeamMember;
        ...
        const total = yield bugReport_model_1.default.countDocuments(query);
        // Build sort object
        const sortObj = {};
        sortObj[sortBy] = sortOrder === 'desc' ? -1 : 1;
        // Execute query with pagination and sorting
        const bugReports = yield bugReport_model_1.default.find(query)
            .sort(sortObj)
            .skip(skip)
            .limit(limit)
            .select(`
      ...
      `)
        ...
        const stats = yield bugReport_model_1.default.aggregate([
            { $match: query },
            {
                $group: {
                    _id: null,
 ...
 * @access Private (Admin only)
 */
const getBugReport = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        // Check if user is authorized (admin/ConnectGo staff)
        if (!((_a = req.user) === null || _a === void 0 ? void 0 : _a.isConnectGoStaff)) {
        ...
        const bugReport = yield bugReport_model_1.default.findById(reportId)
            .populate('reporter', 'name email')
            .populate('assignedTo', 'name email')
        ...
        yield bugReport_model_1.default.findByIdAndUpdate(reportId, {
        ...
        if (error instanceof Error && error.name === 'CastError') {
            const customError = new Error('Invalid bug report ID format');
            customError.statusCode = 400;
 ...
 * @access Private (Admin only)
 */
const updateBugReport = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        // Check if user is authorized (admin/ConnectGo staff)
        if (!((_a = req.user) === null || _a === void 0 ? void 0 : _a.isConnectGoStaff)) {
        ...
        const { status, priority, urgencyLevel, bugType, assignedToTeamMember, sourceOfFeedback, assignedTo, resolved, resolution, 
        // NEW: Verification fields
        verified, verificationDetails, businessImpact, tags, requiresFollowUp, followUpDate, relatedIssues, verifiedByReporter } = req.body;
        // Build update object
        const updateData = {};
        if (status)
        ...
        if (bugType)
            updateData.estimatedEffort = bugType;
        if (assignedToTeamMember !== undefined)
            updateData.assignedToTeamMember = assignedToTeamMember;
        if (sourceOfFeedback)
            updateData.sourceOfFeedback = sourceOfFeedback;
        if (assignedTo)
            updateData.assignedTo = assignedTo;
        if (businessImpact)
            updateData.businessImpact = businessImpact;
        if (tags)
        ...
        if (relatedIssues)
            updateData.relatedIssues = relatedIssues;
        if (verifiedByReporter !== undefined)
            updateData.verifiedByReporter = verifiedByReporter;
        ...
        if (resolved !== undefined) {
            ...
            if (resolved) {
                ...
                if (resolution) {
                    ...
                    const error = new Error('Resolution is required when marking as resolved');
                    error.statusCode = 400;
                    throw error;
                }
            }
        ...
        if (verified !== undefined) {
            ...
            const currentBugReport = yield bugReport_model_1.default.findById(reportId);
            if (!currentBugReport) {
                const error = new Error('Bug report not found');
                error.statusCode = 404;
                throw error;
            ...
            if (verified && !willBeResolved) {
                const error = new Error('Cannot verify a bug that has not been resolved');
                error.statusCode = 400;
                throw error;
            }
            ...
            if (verified) {
                ...
                if (verificationDetails) {
                    ...
                    const error = new Error('Verification details are required when marking as verified');
                    error.statusCode = 400;
                    throw error;
                }
            }
        ...
        const updatedBugReport = yield bugReport_model_1.default.findByIdAndUpdate(reportId, updateData, { new: true, runValidators: true }).populate('reporter', 'name email')
            .populate('assignedTo', 'name email')
            .populate('resolvedBy', 'name email')
            .populate('verifiedBy', 'name email') // NEW: Populate verifiedBy
            .populate('relatedIssues', 'title status priority feedbackType');
        ...
        // Send notification to admin/developer team
        try {
            yield (0, notifications_1.sendBugReportNotification)(updatedBugReport);
        }
        catch (notificationError) {
            console.error('Failed to send bug report notification:', notificationError);
            // Continue even if notification fails
 ...
 * @access Private (Admin only)
 */
const verifyBugReport = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        // Check if user is authorized (admin/ConnectGo staff)
        if (!((_a = req.user) === null || _a === void 0 ? void 0 : _a.isConnectGoStaff)) {
        ...
        if (!verificationDetails || !verificationDetails.trim()) {
            const error = new Error('Verification details are required');
            error.statusCode = 400;
            throw error;
        }
        ...
        if (bugReport.verified) {
            const error = new Error('Bug report is already verified');
            error.statusCode = 400;
            throw error;
        }
        // Update with verification
        const updatedBugReport = yield bugReport_model_1.default.findByIdAndUpdate(reportId, {
            verified: true,
            verifiedAt: new Date(),
            verifiedBy: req.user._id,
            verificationDetails: verificationDetails.trim()
        }, { new: true, runValidators: true }).populate('reporter', 'name email')
            .populate('assignedTo', 'name email')
            .populate('resolvedBy', 'name email')
...
exports.verifyBugReport = verifyBugReport;
/**
 * Get analytics and dashboard data for bug reports
 * @route GET /api/v1/bug-reports/analytics
 * @access Private (Admin only)
 */
const getBugReportAnalytics = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    try {
        // Check if user is authorized (admin/ConnectGo staff)
        if (!((_a = req.user) === null || _a === void 0 ? void 0 : _a.isConnectGoStaff)) {
        ...
        const analytics = yield bugReport_model_1.default.aggregate([
            { $match: dateFilter },
            {
                $facet: {
                    // Overall statistics
...
exports.getBugReportAnalytics = getBugReportAnalytics;
exports.default = {
    submitBugReport: exports.submitBugReport,
    getBugReports: exports.getBugReports,
    getBugReport: exports.getBugReport,
    updateBugReport: exports.updateBugReport,
    verifyBugReport: exports.verifyBugReport, // NEW: Add the new verification endpoint
    getBugReportAnalytics: // NEW: Add the new verification endpoint
    exports.getBugReportAnalytics
};
