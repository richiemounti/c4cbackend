...
exports.runUnifiedMigration = run;
...
const PROJECT_TASK_UPDATES = [
    ...
    },
    {
        ...
        changes: {
            isRequired: false,
            dataType: 'boolean',
            helperText: 'Select Yes if you have shapefiles for the project boundary. A file upload section will appear after selecting Yes, where you can attach your boundary shapefiles.',
        },
    ...
    },
    {
        ...
        changes: {
            dataType: 'boolean',
            helperText: 'Select Yes if you have land agreements or MOUs to upload. A file upload section will appear after selecting Yes. Attach contracts, community agreements, FPIC documentation, or land use approvals — upload as many documents as needed.',
        },
...
// HELPERS
// ─────────────────────────────────────────────────────────────────────────────
function applyChangesToTasks(tasks, updates) {
    const matched = [];
    const missing = [];
    for (const { fieldName, changes } of updates) {
        const idx = tasks.findIndex((t) => t.fieldName === fieldName);
        if (idx !== -1) {
...
function updateExistingDocs(Model, fieldName, changes, dryRun) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = yield Model.updateMany(matchCriteria, { $set: setMap });
...
// ─────────────────────────────────────────────────────────────────────────────
function updateTemplates(dryRun) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log('\n' + '═'.repeat(60));
        console.log('PHASE A — Templates');
        console.log('═'.repeat(60));
        const collection = mongoose_1.default.connection.collection('tasktemplates');
        const projectTemplate = yield collection.findOne({ type: 'project', isActive: true });
        if (!projectTemplate) {
            ...
            if (!dryRun) {
                yield collection.updateOne({ _id: projectTemplate._id }, { $set: { tasks: projectTemplate.tasks, updatedAt: new Date() } });
            ...
            if (missing.length)
                console.log(`  ⚠️   Fields not found in template: ${missing.join(', ')}`);
        }
        const siteTemplate = yield collection.findOne({ type: 'projectSite', isActive: true });
        if (!siteTemplate) {
            ...
            for (const { fieldName, newSortOrder } of SITE_SORT_ORDER) {
                const idx = siteTemplate.tasks.findIndex((t) => t.fieldName === fieldName);
                if (idx !== -1)
                    siteTemplate.tasks[idx].sortOrder = newSortOrder;
            }
            if (!dryRun) {
                yield collection.updateOne({ _id: siteTemplate._id }, { $set: { tasks: siteTemplate.tasks, updatedAt: new Date() } });
...
// ─────────────────────────────────────────────────────────────────────────────
function updateExistingDocuments(dryRun) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log('\n' + '═'.repeat(60));
        ...
        for (const { fieldName, changes, description } of PROJECT_TASK_UPDATES) {
            const count = yield updateExistingDocs(projectSetupTask_model_1.default, fieldName, changes, dryRun);
        ...
        for (const { fieldName, changes, description } of SITE_TASK_UPDATES) {
            const count = yield updateExistingDocs(projectSiteSetupTask_model_1.default, fieldName, changes, dryRun);
            console.log(`  ${dryRun ? '🔍' : '✅'}  ${fieldName}: ${count} doc(s)  — ${description}`);
        }
    });
}
...
// ─────────────────────────────────────────────────────────────────────────────
function reorderSiteTasks(dryRun) {
    return __awaiter(this, void 0, void 0, function* () {
        console.log('\n' + '═'.repeat(60));
        ...
        for (const { fieldName, newSortOrder } of SITE_SORT_ORDER) {
            const matchCriteria = { 'tasks.fieldName': fieldName };
            if (dryRun) {
                const count = yield projectSiteSetupTask_model_1.default.countDocuments(matchCriteria);
                console.log(`  🔍  ${fieldName} → sortOrder ${newSortOrder}  (${count} doc(s) would be updated)`);
            }
            else {
                const result = yield projectSiteSetupTask_model_1.default.updateMany(matchCriteria, {
...
function verifyDocuments() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        console.log('═'.repeat(60));
        const checksProject = [
            { fieldName: 'ecological_zone', check: 'options length', expected: ECOLOGICAL_ZONE_OPTIONS.length, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.length; } },
            { fieldName: 'ecological_zone', check: 'no split "Gobi)" entry', expected: false, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.includes('Gobi)'); } },
        ...
        const checksSite = [
            { fieldName: 'site_ecological_zone', check: 'options length', expected: ECOLOGICAL_ZONE_OPTIONS.length, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.length; } },
            { fieldName: 'site_ecological_zone', check: 'no split "Gobi)" entry', expected: false, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.includes('Gobi)'); } },
            { fieldName: 'site_ecological_zone', check: 'hoverText is empty', expected: '', getter: (t) => t.hoverText },
            { fieldName: 'primary_income_sources', check: 'options length', expected: INCOME_SOURCE_OPTIONS.length, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.length; } },
            { fieldName: 'primary_income_sources', check: 'no split "health)" entry', expected: false, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.includes('health)'); } },
            { fieldName: 'secondary_income_sources', check: 'options length', expected: INCOME_SOURCE_OPTIONS.length, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.length; } },
            { fieldName: 'secondary_income_sources', check: 'isRequired is false', expected: false, getter: (t) => t.isRequired },
            { fieldName: 'crops_grown', check: 'no split horticultural entry', expected: false, getter: (t) => { var _a; return (_a = t.options) === null || _a === void 0 ? void 0 : _a.some((o) => o === 'Horticultural crops (e.g. tomato'); } },
        ...
        const sampleProject = yield projectSetupTask_model_1.default.findOne({});
        if (sampleProject) {
            console.log(`\n[ProjectSetup sample: ${sampleProject._id}]`);
            for (const { fieldName, check, expected, getter } of checksProject) {
                const task = sampleProject.tasks.find((t) => t.fieldName === fieldName);
                if (!task) {
                    console.log(`  ⚠️   ${fieldName} — task not found in document`);
                    continue;
                }
        ...
        const sampleSite = yield projectSiteSetupTask_model_1.default.findOne({});
        if (sampleSite) {
            console.log(`\n[ProjectSiteSetup sample: ${sampleSite._id}]`);
            for (const { fieldName, check, expected, getter } of checksSite) {
                const task = sampleSite.tasks.find((t) => t.fieldName === fieldName);
                if (!task) {
                    console.log(`  ⚠️   ${fieldName} — task not found in document`);
                    continue;
                }
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        console.log(`Scope : ${only !== null && only !== void 0 ? only : 'all phases (A templates + B documents + C reorder + D verify)'}`);
        console.log('');
        try {
            yield (0, mongodb_1.connectToDatabase)();
            if (!only || only === 'templates')
                yield updateTemplates(dryRun);
            if (!only || only === 'documents')
                yield updateExistingDocuments(dryRun);
            if (!only || only === 'reorder')
                yield reorderSiteTasks(dryRun);
            if (only === 'verify' || (!dryRun && !only))
                yield verifyDocuments();
            console.log('\n' + '═'.repeat(60));
...
if (require.main === module)
    run();
//# sourceMappingURL=unifiedMigration.js.map